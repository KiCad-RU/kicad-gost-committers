...
void CSch::DoAlias(wxString aAlias) {
    int i;
    wxString a, n;
    ...
    for (i = 0; i < (int)m_schComponents.GetCount(); i++) {
        if ((m_schComponents[i])->m_objType == 'M') {
            ...
            n.Trim(true);
            n.Trim(false);
            if (((CSchModule *)m_schComponents[i])->m_name.text == n) {
                ((CSchModule *)m_schComponents[i])->m_alias += ' ' + a;
            }
        }
    }
}
...
void CSch::DoLibrary(wxXmlDocument *aXmlDoc, wxStatusBar* aStatusBar, wxString aActualConversion) {
    wxXmlNode *aNode;
    wxString propValue;
    CSchModule *module;
...
void CSch::LinesIntersect(const int aX1, const int aY1, const int aX2, const int aY2, // first line
                    const int aX3, const int aY3, const int aX4, const int aY4, // second line
                    int *aCode, // =0 OK; =1 lines parallel
                    int *aX, int *aY) { // intersection point
    long double a1, a2, b1, b2, c1, c2; // Coefficients of line eqns./
    long double denom;

    a1 = aY2 - aY1;
    b1 = aX1 - aX2;
    c1 = (long double)aX2 * (long double)aY1 - (long double)aX1 * (long double)aY2;  // a1*x + b1*y + c1 = 0 is line 1

    a2 = aY4 - aY3;
    b2 = aX3 - aX4;
    c2 = (long double)aX4 * (long double)aY3 - (long double)aX3 * (long double)aY4;  // a2*x + b2*y + c2 = 0 is line 2
...
bool CSch::IsPointOnLine(int aX, int aY, CSchLine *aLine) {
    ...
    c = (long double)aLine->m_toX * (long double)aLine->m_positionY -
        (long double)aLine->m_positionX * (long double)aLine->m_toY;  // a1*x + b1*y + c1 = 0 is line 1
    px = false; py = false;
...
CSchJunction * CSch::CheckJunction(CSchLine *aSchLine, int aIndex) {
    ...
    for (i = 0; i < (int)m_schComponents.GetCount(); i++) {
        if ((m_schComponents[i])->m_objType == 'L' &&
            ...
            if (aSchLine->m_net == ((CSchLine *)m_schComponents[i])->m_net) {
                // IS JUNCTION ????
                LinesIntersect(aSchLine->m_positionX, aSchLine->m_positionY, aSchLine->m_toX, aSchLine->m_toY,
                               ((CSchLine *)m_schComponents[i])->m_positionX,
                               ((CSchLine *)m_schComponents[i])->m_positionY,
                               ((CSchLine *)m_schComponents[i])->m_toX,
                               ((CSchLine *)m_schComponents[i])->m_toY,
                               &code, &x, &y);
                if (code == 0) {
                    ...
                    if (IsPointOnLine(x, y, aSchLine)) {
                        ...
                        for (j = 0; j < (int)m_schComponents.GetCount(); j++) {
                            if ((m_schComponents[j])->m_objType == 'L'
                                 && p) { // not  for itself
                                if (((CSchLine *)m_schComponents[j])->m_net != ((CSchLine *)m_schComponents[i])->m_net)
                                    if (IsPointOnLine(x, y, (CSchLine *)m_schComponents[j]))
                                           p = false; // NOT POINT - net cross
                            }
                        }
                        if (p) result = new CSchJunction(x, y, aSchLine->m_net);
                    }
                }
            }
        }
...
void CSch::Parse(wxStatusBar* aStatusBar, wxString aXMLFileName, wxString aActualConversion) {
    ...
    if (!xmlDoc.Load(aXMLFileName)) return;
    ...
    if (aNode) {
        ...
        for (i = 0; i < (int)m_schComponents.GetCount(); i++) {
            ...
            if (schComp->m_objType == 'L') {
                ((CSchLine *)schComp)->m_toY -= m_sizeY;
                ((CSchLine *)schComp)->m_labelText.textPositionY -= m_sizeY;
                ((CSchLine *)schComp)->m_toX -= m_sizeX;
                ((CSchLine *)schComp)->m_labelText.textPositionX -= m_sizeX;
            }
        }
        ...
        if (m_sizeX < 11700) m_sizeX = 11700;
        if (m_sizeY < 8267) m_sizeY = 8267;
...
void CSch::WriteToFile(wxString aFileName, char aFileType) {
    wxFile f;
    int i;
    ...
    if (aFileType == 'S') {
        ...
        for (i = 0; i < (int)m_schComponents.GetCount(); i++) {
            if (m_schComponents[i]->m_objType == 'L')
                ((CSchLine *)m_schComponents[i])->WriteLabelToFile(&f, aFileType);
        }
        // Symbols
        for (i = 0; i < (int)m_schComponents.GetCount(); i++) {
