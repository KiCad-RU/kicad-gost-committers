...
void SCH::LinesIntersect(const int aX1, const int aY1, const int aX2, const int aY2, // first line
                    ...
                    if (IsPointOnLine(x, y, aSchLine)) {
                        ...
                        for (j = 0; j < (int)m_schComponents.GetCount(); j++) {
                            if ((m_schComponents[j])->m_objType == 'L'
                                 && p) { // not  for itself
                                if (((SCH_LINE *)m_schComponents[j])->m_net != ((SCH_LINE *)m_schComponents[i])->m_net)
                                    if (IsPointOnLine(x, y, (SCH_LINE *)m_schComponents[j]))
                                           p = false; // NOT POINT - net cross
                            }
                        }
                        if (p) result = new SCH_JUNCTION(x, y, aSchLine->m_net);
                    }
                }
            }
        }
...
void SCH::WriteToFile(wxString aFileName, char aFileType) {
    ...
    if (aFileType == 'L') {
        ...
        for (i = 0; i < (int)m_schComponents.GetCount(); i++) {
            if (m_schComponents[i]->m_objType == 'M') {
                f.Write(wxT("#\n"));
                f.Write(wxT("$CMP ") + ValidateName(((SCH_MODULE *)m_schComponents[i])->m_name.text) + wxT("\n"));
                f.Write(wxT("D ") + ((SCH_MODULE *)m_schComponents[i])->m_moduleDescription + wxT("\n"));
