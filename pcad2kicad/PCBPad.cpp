...
wxString CPCBPad::KiCadLayerMask(wxString mask, int l) {
    wxString newmask;
    int i;
...
void CPCBPad::Parse(wxXmlNode *aNode, wxString aDefaultMeasurementUnit, wxString aActualConversion) {
    wxXmlNode *lNode;
    long num;
...
void CPCBPad::WriteToFile(wxFile *aFile, char aFileType, int aRotation) {
    CPCBPadShape *padShape;
    ...
    if (aFileType == 'P') { // PCB
        for (i = 0; i < (int)m_shapes.GetCount(); i++) {
            ...
            if (padShape->m_width > 0 || padShape->m_height > 0) { // maybe should not to be filtered ????
                s = wxT("3") ; // default
                aFile->Write(wxT("Po ") + s + wxString::Format(" %d %d %d %d %d %d\n", m_positionX, m_positionY,
            ...
            if (padShape->m_width > 0 || padShape->m_height > 0) { // maybe should not to be filtered ????
                ...
                aFile->Write(wxT("Sh \"") + m_name.text + wxT("\" ") + s +
                         wxString::Format(" %d %d 0 0 %d\n",
                             padShape->m_width, padShape->m_height, m_rotation + aRotation)); // Name, Shape, Xsize Ysize Xdelta Ydelta Orientation
