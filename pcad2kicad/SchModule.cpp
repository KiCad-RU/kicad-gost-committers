...
void CSchModule::Parse(wxXmlNode *aNode, wxStatusBar* aStatusBar,
...
void CSchModule::SetPinProperties(wxXmlNode *aNode, int aSymbolIndex,
    wxString aDefaultMeasurementUnit, wxString aActualConversion)
{
    CSchPin *schPin;
    int i;
    ...
    for (i = 0; i < (int)m_moduleObjects.GetCount(); i++) {
        if ((m_moduleObjects[i])->m_objType == 'P') {
            schPin = (CSchPin *)m_moduleObjects[i];
...
void CSchModule::FindAndProcessSymbolDef(wxXmlNode *aNode, int aSymbolIndex,
    wxString aDefaultMeasurementUnit, wxString aActualConversion)
{
...
void CSchModule::WriteToFile(wxFile *aFile, char aFileType) {
    int i, symbolIndex;
